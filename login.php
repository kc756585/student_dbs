	<?php include("01_head.php"); ?>
			<form id="form1" class="sui-form form-horizontal sui-validate" action="login-save-ajax.php" method="post">
...
$("button[type=submit]").bind("click",function(){
	...
	$.ajax({
		...
		success:function(data,textStatus){
			console.log(data);
			if (data.code==200) {
				$(".pp").html("登录成功").slideDown(200).animate({"color":"black"},2000,function(){
					window.location.href=("index.php");
				});
			}
			if (data.code==20001) {
				$(".pp").html("密码错误").slideDown(200);
			}
			if (data.code==20004) {
				$(".pp").html("邮箱填写错误").slideDown(200);
			}
			$(".pp").slideUp(400);
